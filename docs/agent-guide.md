...
This page is a practical, self-contained guide for creating a game with **Sucata**, in the spirit of Love2D/Godot. It's aimed at whoever — human or AI coding agent — is writing a *game* on top of Sucata, condensing everything needed to be productive on one page, with links to the full reference for exhaustive per-function detail. It's the same guide shipped as `AGENT.md` in the [sucata](https://github.com/sucata-engine/sucata) repository, for use by coding agents working directly in a project's source tree.
...
| Command | Purpose |
|---|---|
| `sucata run <main.lua> [--entity <file>]` | Run a game (or a single entity file, for isolated testing) through the engine in dev mode. |
...
**`sucata.window`** — `get/set_mouse_lock(bool)`, `get/set_mouse_visible(bool)`, `get/set_window_title(str)`, `get/set_window_size(w,h)`, `get/set_fullscreen(bool)`, `get/set_vsync(n)`, `quit()`, `show_debug_info(bool)`, `get/set_keep_aspect(0|1|2)`, `get/set_window_icon(path)`, `get/set_cursor(name)`, `on_init(callback)`, `on_exit(callback)`. [Full reference](references/sucata.window.md)
